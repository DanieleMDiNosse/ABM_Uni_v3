...
- **Rich agent roster**:
  ...
  - **Jiter**: MEV-style *Just-In-Time* LP that targets the single largest swap in the mempool and mints/burns a single-tick position around it.
...
- **Types**:
  ...
  - JIT LP “Jiter” agent: An MEV-style *Just-in-Time* LP that can join blocks with Bernoulli arrival probability `p_jit`. At each block where it arrives, Jiter observes the full mempool and targets the single largest swap intent by input size (excluding the arbitrage intent). It mints a single-tick position aligned with the current active tick immediately before the swap and burns it immediately after. Liquidity is chosen as the maximum of: (i) the amount needed to own a fraction `liquidity_perc_jit` of the active liquidity in that tick and (ii) the minimum liquidity needed to keep the targeted swap within that tick. Jiter is flash-funded and pays a flash-loan fee (using the same `flash_loan_fee` parameter as the arbitrageur), valued at the mint-time CEX price snapshot.
