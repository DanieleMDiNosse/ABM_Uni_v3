...
- **Discrete blocks with micro-steps**: one simulation step `t` is one “block”. Inside each block there are `block_time = B` *micro-steps* where only the CEX price diffuses and trader intents arrive.
  - In the intended interpretation, **one micro-step is one second**, so one block lasts `B` seconds.
...
5) **Micro-step loop (mempool intake; DEX frozen)**
   - Repeat `B = block_time` times:
     - Sample smart-router arrivals per micro-step:
       - if `smart_trades_per_second` is set: `n_smart ~ Poisson(smart_trades_per_second)`
       - else (legacy): `n_smart ~ Poisson(smart_trades_per_block / B)`
     - Sample noise-trader arrivals per micro-step:
       - if `noise_trades_per_second` is set: `n_noise ~ Poisson(noise_trades_per_second)`
       - else (legacy): `n_noise ~ Poisson(noise_trades_per_block / B)`
...
- **Arrival per micro-step**:
  - Smart-router:
    - if `smart_trades_per_second` is set: `n_smart ~ Poisson(smart_trades_per_second)`
    - else (legacy): `n_smart ~ Poisson(smart_trades_per_block / block_time)`
  - Noise trader:
    - if `noise_trades_per_second` is set: `n_noise ~ Poisson(noise_trades_per_second)`
    - else (legacy): `n_noise ~ Poisson(noise_trades_per_block / block_time)`
...
- **Arrival process**:
  - During each micro-step:
    - if `smart_trades_per_second` is set: `N ~ Poisson(smart_trades_per_second)`
    - else (legacy): `N ~ Poisson(smart_trades_per_block / block_time)`
