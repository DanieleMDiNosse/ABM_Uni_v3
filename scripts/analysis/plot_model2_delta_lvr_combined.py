#!/usr/bin/env python3
"""Build the combined Model 2 block-size ΔLVR figure from exact summaries.

The source of truth is the production ``dLVR_summary.csv`` files under
``/mnt/external/scenarios``.  The companion table written by this script keeps
all summary statistics and records the exact source CSV for every plotted row.
"""
...
DEFAULT_DATA_PATH = _REPO_ROOT / "paper" / "tables" / "model2_delta_lvr_blocksize_values.csv"
DEFAULT_STYLE_PATH = _REPO_ROOT / "paper" / "figure_style.yml"
DEFAULT_OUTPUT_PATH = _REPO_ROOT / "paper" / "images" / "model2_delta_lvr_blocksize_combined.png"
DEFAULT_MANIFEST_PATH = _REPO_ROOT / "paper" / "tables" / "model2_delta_lvr_blocksize_combined_manifest.json"
...
def load_paper_style(path: Path) -> Dict[str, Any]:
    return load_figure_style(path, DEFAULT_STYLE, figure_key="model2_delta_lvr_blocksize_combined")
...
def rebuild_data_from_source_csvs(source_root: Path, output_path: Path) -> pd.DataFrame:
    ...
    for schedule, cfg in SCHEDULES.items():
        ...
        df["median_delta_lvr"] = df["median"]
        df["median_delta_lvr_x1e3"] = df["median_delta_lvr"] * 1000.0
...
def load_delta_lvr_data(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Missing ΔLVR table: {path}. Run with --rebuild-data-from-source-csv first.")
    df = pd.read_csv(path)
    required = {"block_time", "fee_schedule", "cohort", "median_delta_lvr", "median_delta_lvr_x1e3"}
    missing = sorted(required - set(df.columns))
    if missing:
        raise ValueError(f"ΔLVR table {path} is missing columns: {missing}")
    df = df.copy()
    df["block_time"] = df["block_time"].astype(int)
    df["median_delta_lvr"] = df["median_delta_lvr"].astype(float)
    df["median_delta_lvr_x1e3"] = df["median_delta_lvr_x1e3"].astype(float)
...
def build_figure(df: pd.DataFrame, style: Mapping[str, Any]) -> go.Figure:
    ...
    for col_idx, cohort in enumerate(COHORTS, start=1):
        ...
        for schedule in SCHEDULES:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=plot_df["median_delta_lvr_x1e3"],
                    mode="lines+markers",
                    name=str(cfg["label"]),
                    ...
                    line=dict(
                        color=str(cfg["color"]),
                        width=float(line_cfg.get("width", 4.5)),
                        dash=str(cfg["dash"]),
                    ),
                    marker=dict(
                        color=str(cfg["color"]),
                        size=float(line_cfg.get("marker_size", 10)),
                    ),
                    hovertemplate=(
                        f"{COHORTS[cohort]}<br>"
                        f"{cfg['label']}<br>"
                        "B=%{x}<br>median ΔLVR=%{customdata:.4g}<extra></extra>"
                    ),
                    customdata=plot_df["median_delta_lvr"],
                ),
                row=1,
                col=col_idx,
            )
        ...
        fig.update_yaxes(
            title_text="Median ΔLVR per block (×10⁻³)" if col_idx == 1 else "",
            title_font=dict(size=int(font["axis_title_size"])),
            tickfont=dict(size=int(font["tick_size"])),
            zeroline=False,
            row=1,
...
def write_manifest(
    *,
    ...
    manifest = {
        ...
        "source_note": (
            "All plotted ΔLVR medians are read from exact production Model 2 dLVR_summary.csv "
            "files under /mnt/external/scenarios. No PNG digitization is used for this figure. "
            "The figure plots 1000 × the median column from each source summary."
        ),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Rebuild the companion CSV from exact /mnt/external/scenarios dLVR_summary.csv files before plotting.",
    )
...
def main() -> None:
    ...
    if args.rebuild_data_from_source_csv:
        ...
        df = load_delta_lvr_data(data_path)
